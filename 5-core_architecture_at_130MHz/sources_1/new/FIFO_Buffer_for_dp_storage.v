...
module FIFO_Buffer_for_dp_storage #(parameter DATA_WIDTH = 131, // number of bits in a word
...
output wire [DATA_WIDTH-1:0] read_data_1, read_data_2, read_data_3, read_data_4);                                 
    ...
    register_file_for_FIFO_Buffer #(.DATA_WIDTH(DATA_WIDTH), .ADDR_WIDTH(ADDR_WIDTH))
    register_file_1(.clk(clk), 
     .write_enable(write_enable),
     .write_addr(write_addr), .read_addr(read_addr),
     .write_data(write_data_1), .read_data(read_data_1));       
     
    register_file_for_FIFO_Buffer #(.DATA_WIDTH(DATA_WIDTH), .ADDR_WIDTH(ADDR_WIDTH)) register_file_2(.clk(clk), 
     .write_enable(write_enable),
     .write_addr(write_addr), .read_addr(read_addr),
     .write_data(write_data_2), .read_data(read_data_2));  
    register_file_for_FIFO_Buffer #(.DATA_WIDTH(DATA_WIDTH), .ADDR_WIDTH(ADDR_WIDTH)) register_file_3(.clk(clk), 
     .write_enable(write_enable),
     .write_addr(write_addr), .read_addr(read_addr),
